...
// Reads weights/topology from bundle.js, reimplements the CA step in JS.
"use strict";

const N    = BUNDLE.n;
const C    = BUNDLE.channels;
const AIDX = BUNDLE.alive_alpha_idx;   // alpha channel index
const pos  = Float32Array.from(BUNDLE.pos);
const off  = Int32Array.from(BUNDLE.csr_off);
const src  = Int32Array.from(BUNDLE.csr_src);
// weight matrices stored flat, row-major. w1: (128, 3C)  w2: (C, 128)
const W1 = Float32Array.from(BUNDLE.w1);
const b1 = Float32Array.from(BUNDLE.b1);
const W2 = Float32Array.from(BUNDLE.w2);
const H  = 128;                          // hidden width
const TGT = Float32Array.from(BUNDLE.target);
const SEED = BUNDLE.seed;

// ---- state ----
let x = new Float32Array(N * C);         // node states
let t = 0, running = true, stepsPerFrame = 2;
let ghost = true;
let brushR = 0.08;
...
function toPx(p) { return p * RES; }     // positions already in [0,1]

function seed() {
  x.fill(0);
  x[SEED * C + AIDX] = 1;                // alive marker in seed node
  for (let c = AIDX; c < C; c++) x[SEED * C + c] = 1;
  t = 0;
}

// ---- one CA step: alive-mask, message-pass perception, MLP update ----
function step() {
  // (1) pre-update alive mask: node lives if it or any neighbor has alpha>0.1
  ...
  // (2) perception + update for every node
  const xn = new Float32Array(N * C);
  const z = new Float32Array(3 * C);
  const h = new Float32Array(H);
  for (let i = 0; i < N; i++) {
    // aggregate neighbor states: mean_n and mean(n - self)
    ...
    for (let c = 0; c < C; c++) {
      ...
      const mn = s / d;
      z[c]       = x[i * C + c];          // own
      z[C + c]   = mn;                    // mean neighbor
      z[2*C + c] = mn - x[i * C + c];     // mean (neighbor - own)
    }
    // MLP: h = relu(W1 z + b1)
    for (let k = 0; k < H; k++) {
      let acc = b1[k];
...
function loss() {
  ...
  for (let i = 0; i < N; i++)
    for (let c = 0; c < 4; c++) {
      const d = x[i*C+c] - TGT[i*4+c]; s += d * d;
    }
  return s / (N * 4);
}
function alivePct() {
...
function paintDamage(ev, radius) {
  ...
  const py = 1 - (ev.clientY - r.top) / r.height;   // flip y
